...
router.get('/schedule.hbs',(req,res)=>{
    db.query('SELECT name FROM emp',(err,result)=>{
        res.render('schedule',{faclist:result});
    })
        ...
        db.query('SELECT * FROM faclog',(err,results)=>{
            console.log(results)
             db.query('SELECT notification FROM notify WHERE name=?',[results[0].name],(error,result)=>{
                db.query('SELECT name FROM emp WHERE role="AF"',(err,result1)=>{
                    res.render('facultyhome',{ facname:result1,name:results[0].name,userid:results[0].userid,phno:results[0].phno,dept:results[0].dept,gmail:results[0].gmail,notific:result})
                })
...
router.get('/scheduleshow.hbs',(req,res)=>{
    db.query('SELECT name,room,date FROM schedule',(err,results)=>{
        if(err) throw err;
        console.log(results);
        res.render('scheduleshow',{userdata: results});
    });
})
    ...
    db.query('SELECT * FROM faclog',(err,results)=>{
        ...
        db.query('SELECT room,date FROM schedule WHERE name=?',[name1],(error,result)=>{
            ...
            if(result<=0){
                ...
                res.render('showsh',{noroom:"No Schedule"});
            }
            else{
